...
export default function EasyMiningUnit() {
  ...
  const engines = [
    {
      id: "forex", name: "Forex Trading Engine", icon: "💱", color: "#3b82f6", status: "5 LIVE", profit: "+$31.19",
      details: "EUR/USD BUY 0.10 +$8.32 • GBP/USD SELL 0.05 +$5.10 • USD/JPY BUY 0.08 +$3.25 • XAU BUY 0.03 +$1.92 • BTC SELL 0.001 +$12.60",
      work: "Trend+ Swing+ Breakout+ Carry+ Scalping = 5 Strategies LIVE per second • MA20 1.0831 MA50 1.0819",
      easy: "One Click → Auto trades forex 24/7 → Profit auto-credited"
    },
    {
      id: "crypto", name: "Crypto Mining Engine", icon: "⛏️", color: "#f59e0b", status: "2.4 TH/s", profit: "+$12.40/day",
      details: "PoW: Hash puzzles • PoC: 4.2TB disk • PoB: 42 TRX burned • PoH: 65k TPS • Block #64821903",
      work: "PoW secures TXID • PoC uses storage • PoB burns coins • PoH timestamps • All mine TRX/BTC LIVE",
      easy: "One Click → Starts all 7 mining techniques → Hash rate LIVE"
    },
    {
      id: "staking", name: "Staking/Yield Engine", icon: "💎", color: "#10b981", status: "Staked", profit: "8.4% APY",
      details: `Staked $${bal.toFixed(2)} • PoS validators • Compounding • Yield LIVE • Daily: +$0.42`,
      work: "PoS staking modules enable compounding profits • Validators earn yields • Auto-compound every second",
      easy: "One Click → Stake USDT → 8.4% APY auto-compounds → Profit every second"
    },
    {
      id: "compliance", name: "Compliance Engine", icon: "🛡️", color: "#ef4444", status: "100% OK", profit: "ESG 100%",
      details: "PoA trusted validators • SumSub KYC • Chainalysis AML Score <75 • World Bank Grade",
      work: "PoA tracks ESG supply chain transparency • AML compliance • KYC verification • Risk score LIVE",
      easy: "One Click → Checks all deposits • AML <75 OK → ESG 100% compliant"
    },
    {
      id: "analytics", name: "Analytics Engine", icon: "📊", color: "#8b5cf6", status: "LIVE", profit: "1,247 TX",
      details: "On-chain analytics • Whale tracking • TRC20 TATeM34UJsHhnADWrjemjYXdrHiyuPrFra • Risk LIVE",
      work: "PoW feeds provide TXID + block rewards for dashboards • Whale alerts • Profitability metrics real time",
      easy: "One Click → Tracks all TRC20 deposits → Whale alerts → Dashboard LIVE"
    },
    {
      id: "dashboard", name: "Investor Dashboard", icon: "👁️", color: "#ec4899", status: "LIVE", profit: `$${totalProfit.toFixed(2)} Net`,
      details: `Balance $${bal.toFixed(2)} • Net +$${totalProfit.toFixed(2)} • 50/50 split • Real time {time}`,
      work: "Displays profitability + sustainability metrics in real time • World Bank grade transparency • All engines combined",
      easy: "One Click → Shows all profits • Sustainability • Transparency • Investor view LIVE"
    },
  ];
  ...
  return (
    <div style={{ background: "#0f172a", minHeight: "100vh", color: "#fff", fontFamily: "system-ui", padding: 16 }}>
      <div style={{ maxWidth: 1450, margin: "0 auto" }}>
        ...
        {/* 6 ENGINES EASY MINING GRID */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginTop: 16 }}>
          {engines.map(e => (
            <div key={e.id} onClick={() => setActive(e.id)} style={{ background: active === e.id? e.color : "#1e293b", border: `3px solid ${active === e.id? e.color : "#334155"}`, borderRadius: 12, padding: 14, cursor: "pointer", transform: active === e.id? "scale(1.02)" : "scale(1)", transition: "0.2s" }}>
              ...
              <div style={{ fontSize: 8, marginTop: 6, opacity: 0.8 }}><b>WORK:</b> {e.work.slice(0, 80)}...</div>
            </div>
          ))}
        </div>

        {/* ACTIVE ENGINE + HOW THEY WORK TOGETHER */}
          ...
          <div style={{ background: "#fff", color: "#000", borderRadius: 12, padding: 18 }}>
            ...
            <div style={{ marginTop: 14, border: "2px solid #fbbf24", borderRadius: 10, overflow: "hidden" }}>
              ...
              <div style={{ fontSize: 9 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr", padding: 8, borderBottom: "1px solid #f1f5f9" }}><div><b>⛏️ Crypto Mining</b></div><div>PoW+PoS+PoA+PoC+PoB+PoH • 2.4 TH/s</div><div>Secures transactions • Block # {Math.floor(Math.random()*1000)+64821000} • {time}</div></div>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            <div style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 12, padding: 14 }}>
              ...
              <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
                <div style={{ background: "#0f172a", padding: 8, borderRadius: 6, display: "flex", justifyContent: "space-between", fontSize: 10 }}><span>💱 Forex</span><b style={{ color: "#10b981" }}>+$31.19 LIVE {time.slice(0,5)}</b></div>
                <div style={{ background: "#0f172a", padding: 8, borderRadius: 6, display: "flex", justifyContent: "space-between", fontSize: 10 }}><span>⛏️ Mining</span><b style={{ color: "#10b981" }}>2.4 TH/s {time.slice(0,5)}</b></div>
                <div style={{ background: "#0f172a", padding: 8, borderRadius: 6, display: "flex", justifyContent: "space-between", fontSize: 10 }}><span>💎 Staking</span><b style={{ color: "#10b981" }}>8.4% APY {time.slice(0,5)}</b></div>
                <div style={{ background: "#0f172a", padding: 8, borderRadius: 6, display: "flex", justifyContent: "space-between", fontSize: 10 }}><span>🛡️ Compliance</span><b style={{ color: "#3b82f6" }}>100% OK {time.slice(0,5)}</b></div>
                <div style={{ background: "#0f172a", padding: 8, borderRadius: 6, display: "flex", justifyContent: "space-between", fontSize: 10 }}><span>📊 Analytics</span><b style={{ color: "#8b5cf6" }}>LIVE {time.slice(0,5)}</b></div>
                <div style={{ background: "#0f172a", padding: 8, borderRadius: 6, display: "flex", justifyContent: "space-between", fontSize: 10 }}><span>👁️ Dashboard</span><b style={{ color: "#ec4899" }}>LIVE {time.slice(0,5)}</b></div>
              </div>
            </div>
